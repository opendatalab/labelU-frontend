...
const CreateTask = (props: any) => {
  ...
  const nextWhen0 = async function () {
    ...
    let isTaskDescriptionOver = commonController.isOverFontCount(
      taskDescription,
      500
    );
    ...
    try {
      ...
      if (haveConfigedStep !== 0) {
        res = await updateTaskConfig(taskId, {
          name: taskName,
          description: taskDescription,
          tips: taskTips,
        });
      } else {
        res = await submitBasicConfig({
          name: taskName,
          description: taskDescription,
          tips: taskTips,
        });
  ...
  const nextWhen1 = async function () {
    ...
    if (
      newSamples.length === 0 &&
      (taskStatus === 'DRAFT' || taskStatus === 'IMPORTED' || !taskStatus)
    ) {
      commonController.notificationWarnMessage(
        { message: '请导入数据,再进行下一步操作' },
        1
      );
      return false;
    }
      ...
      getTask(taskId)
        .then((res: any) => {
          if (res.status === 200) {
            dispatch(
                // @ts-ignore
              updateTask({ data: res.data.data, configStatus: currentStatus })
            );
    ...
    <div className={currentStyles.outerFrame}>
      <div className={currentStyles.stepsRow}>
        ...
        <div className={currentStyles.right}>
          <div
            className={`${commonStyles.cancelButton}  ${currentStyles.cancelButton}`}
