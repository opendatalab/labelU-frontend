...
const Step = (props : any)=>{
    ...
    return (<div className = {currentStyles.outerFrame} onClick={ turnToOutlet } >
        {configStep === ordinalNumber - 2
        && haveConfigedStep < ordinalNumber
        && <React.Fragment><div className = {currentStyles.iconHighlight}>
            {ordinalNumber}
        </div>
            <div className = {currentStyles.titleHighlight}> {title} </div></React.Fragment>}
        ...
        { configStep === ordinalNumber - 2
        && haveConfigedStep >= ordinalNumber
        && <React.Fragment><div className = {currentStyles.iconCheckHighlight}>
            <CheckOutlined />
        </div>
            <div className = {currentStyles.titleHighlight}> {title} </div></React.Fragment>}
        ...
        { configStep !== ordinalNumber - 2
        && haveConfigedStep >= ordinalNumber
        && <React.Fragment><div className = {currentStyles.iconCheck}>
            <CheckOutlined />
        </div>
            <div className = {currentStyles.title}> {title} </div></React.Fragment>}
